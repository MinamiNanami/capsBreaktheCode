...
<div class="flex flex-col h-full md:flex-row">
    ...
    <div class="flex-1 bg-white p-2 rounded shadow mr-2">
        ...
        <div id="itemGrid" class="overflow-y-auto h-[calc(100vh-250px)]">
...
<script>
    ...
    function printReceipt() {
        ...
        <head>
            <title>Receipt</title>
            <style>
                body { font-family: Arial; max-width:400px; margin:0 auto; padding:20px; line-height:1.4; color:#333; }
                .header{text-align:center;margin-bottom:30px;}
                .logo{width:60px;height:60px;background:#f0f0f0;border-radius:50%;margin:0 auto 15px;display:flex;align-items:center;justify-content:center;font-weight:bold;font-size:24px;color:#666;}
                ...
                .customer-name{font-size:14px;margin-bottom:10px;}
        ...
        <body>
            ...
            <div class="customer-info"><div class="customer-name">👤 ${customerName}</div></div>
            <div class="items-summary">${cart.length} items (Qty.: ${cart.reduce((total, item) => total + item.qty, 0)})</div>
            <div class="items-list">
                ${cart.map(item => `<div class="item-row"><div class="item-qty">${item.qty}×</div><div class="item-name">${item.name}</div><div class="item-price">₱${(item.qty*item.price).toFixed(2)}</div></div>`).join('')}
            </div>
            <div class="totals-section">
                <div class="total-row"><span>Subtotal:</span><span>₱${subtotal.toFixed(2)}</span></div>
                ${serviceFee>0?`<div class="total-row"><span>Service Fee:</span><span>₱${serviceFee.toFixed(2)}</span></div>`:''}
                ${discountPercent>0?`<div class="total-row"><span>Discount (${discountPercent}%):</span><span>-₱${discountAmount.toFixed(2)}</span></div>`:''}
                <div class="total-row final-total"><span>Total:</span><span>₱${finalTotal.toFixed(2)}</span></div>
            </div>
            <div class="footer">
